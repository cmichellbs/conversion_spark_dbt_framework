  ...
  if object_id ('"dbo_mig"."cad_unidade_comercial__dbt_tmp_temp_view"','V') is not null
      begin
      drop view "dbo_mig"."cad_unidade_comercial__dbt_tmp_temp_view"
      end
  ...
  USE [papakura_20221223];
  if object_id ('"dbo_mig"."cad_unidade_comercial__dbt_tmp"','U') is not null
      begin
      drop table "dbo_mig"."cad_unidade_comercial__dbt_tmp"
      end


   USE [papakura_20221223];
   EXEC('create view "dbo_mig"."cad_unidade_comercial__dbt_tmp_temp_view" as
    with gpfat as (
    select  
cast(case 
	when cast(substring(r.RSD$READSEQ, 1,3) as varchar) = ''000'' then ''001'' 
    when cast(substring(r.RSD$READSEQ, 1,3) as varchar) = ''HOL'' then ''099''
    else cast(substring(r.RSD$READSEQ, 1,3) as varchar)
end as integer ) as id_grupo_faturamento,  
i.INSTALL as INSTALL
from "papakura_20221223"."dbo"."GBREADSEQ" r 
inner join "papakura_20221223"."dbo"."INSTALL" i on cast(i.INSTALL as varchar) = cast(r.RSD$INSTALL as varchar)

)

,CORRENTE_INTERM AS(SELECT *, ROW_NUMBER() OVER(PARTITION BY CINSTALL ORDER BY CONSUMERNO DESC) RN FROM "papakura_20221223"."dbo"."CONSUMER")
,CORRENTE AS (SELECT * FROM CORRENTE_INTERM WHERE RN =1)

,loc as (select *  from "papakura_20221223"."dbo_mig"."tab_localizacao" where IS_QUADRA = 1)


,nr_loc2 as (select *, 
cast(case
    when RSD$KEYNUM  is null then CASE WHEN RSD$READSEQ = ''HOLD'' THEN ''9696'' ELSE concat(''0'',SUBSTRING(cast(RSD$READSEQ as varchar),1,3)) END
 ...
 from "papakura_20221223"."dbo"."GBREADSEQ" 
) 
...
,CONSUMER_INTERM AS(SELECT *,ROW_NUMBER() OVER(PARTITION BY CINSTALL ORDER BY CONSUMERNO DESC) RN FROM "papakura_20221223"."dbo"."CONSUMER" )

,CONSUMER_FINAL AS (SELECT * FROM CONSUMER_INTERM WHERE RN = 1)

,CCUC AS (SELECT * FROM "papakura_20221223"."dbo_mig"."cad_cliente_unidade_comercial" where CH_ATIVO = 1 AND  CH_TIPO_CLIENTE =1   )
...
(select c.CINSTALL  from "papakura_20221223"."dbo"."LEDGERPAY" l 
inner join "papakura_20221223"."dbo"."CONSUMER" c on c.CACCT = l.LP$LEDGER ) then 1 else 2 end as CH_TIPO_COBRANCA,
...
SCE.CH_TIPO_CATEGORIA_TARIFA as CH_TIPO_CATEGORIA_TARIFA,
...
from "papakura_20221223"."dbo"."INSTALL" INSTALL
left JOIN "papakura_20221223"."dbo_mig"."cad_endereco" ENDERECO on INSTALL.INSTALL = ENDERECO.ACTID
...
LEFT JOIN "papakura_20221223"."dbo_mig"."stage_contrato" AS SC ON CAST(SC.CCON_CONSUMER AS VARCHAR)= CAST(CONS.MIG_PK_TEMP AS VARCHAR)
left join "papakura_20221223"."dbo_mig"."med_rota_leitura_agua_sequencia" MRLAS ON MRLAS.CH_MATRICULA_UNIDADE = INSTALL.INSTALL
left join "papakura_20221223"."dbo_mig"."cad_unidade_orgao_centralizador" CUOC ON CUOC.CH_MATRICULA_UNIDADE = INSTALL.INSTALL
LEFT JOIN "papakura_20221223"."dbo_mig"."stage_cad_economia" SCE ON SCE.CH_MATRICULA_UNIDADE = INSTALL.INSTALL 
LEFT JOIN "papakura_20221223"."dbo_mig"."tab_categoria_tipo_tarifa" TCTT ON TCTT.ID_CATEGORIA_TIPO_TARIFA = SCE.ID_CATEGORIA_TIPO_TARIFA
...
select * from final2 --where rn = 1
   ...
   SELECT * INTO "papakura_20221223"."dbo_mig"."cad_unidade_comercial__dbt_tmp" FROM
    "papakura_20221223"."dbo_mig"."cad_unidade_comercial__dbt_tmp_temp_view"
  ...
  if EXISTS (
        SELECT * FROM
        sys.indexes WHERE name = 'dbo_mig_cad_unidade_comercial__dbt_tmp_cci'
        AND object_id=object_id('dbo_mig_cad_unidade_comercial__dbt_tmp')
    )
  DROP index "dbo_mig"."cad_unidade_comercial__dbt_tmp".dbo_mig_cad_unidade_comercial__dbt_tmp_cci
  CREATE CLUSTERED COLUMNSTORE INDEX dbo_mig_cad_unidade_comercial__dbt_tmp_cci
    ON "dbo_mig"."cad_unidade_comercial__dbt_tmp"
